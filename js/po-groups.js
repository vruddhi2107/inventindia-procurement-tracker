...
// Engineer's original attachments, from procurement_requests.attachments
// (jsonb, saved at PR submission — see engineer.html's create-request
// handler). The legacy PR modal (procurement.html's openPR) normalizes and
// shows these via _buildAttsSection, but openPRNewModel never fetched or
// rendered them at all, so any image/file the engineer attached at
// submission was invisible for every role on a part-level-tracked PR,
// permanently — not just past some phase. Self-contained (doesn't reuse
// procurement.html's _buildAttsSection, which isn't in scope on the
// engineer/pm/master/accounts pages that also open this modal).
function renderEngineerAttachments(pr) {
  const atts = Array.isArray(pr.attachments) ? pr.attachments : [];
  if (!atts.length) return '';
  const cards = atts.map(a => {
    const url = a.url || a.data;
    if (!url) return '';
    const name = a.name || 'Attachment';
    const fi = _regFile(url, name);
    const isImg = /\.(png|jpe?g|gif|webp)$/i.test(name) || (a.type || '').startsWith('image/') || url.startsWith('data:image');
    return `<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:white;border:1px solid var(--border);border-radius:var(--radius-sm)">
      ${isImg ? `<img src="${url}" style="width:44px;height:44px;object-fit:cover;border-radius:6px;border:1px solid var(--border)" onerror="this.style.display='none'"/>` : `<span style="font-size:1.4rem">📄</span>`}
      <span style="flex:1;font-size:0.8rem;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${name}</span>
      <button class="btn btn-secondary btn-sm" onclick="_previewByIdx(${fi})">👁 Preview</button>
      <button class="btn btn-secondary btn-sm" onclick="_downloadByIdx(${fi})">⬇ Download</button>
    </div>`;
  }).join('');
  return `<div style="margin-bottom:14px">
    <div class="section-title" style="margin-bottom:8px">📎 Attachments from Engineer</div>
...
async function openPRNewModel(id) {
  ...
  showLoader(true);
  // PR_LIST_COLUMNS deliberately excludes `attachments` (see its definition
  // in supabase-config.js) because it's meant for list rows, not detail
  // views — but this single-row fetch IS the detail view, and nothing else
  // was ever adding `attachments` back in. That's the actual reason the
  // engineer's originally-attached image never appeared here: the column
  // was never being requested from Supabase in the first place, on any
  // role's page, regardless of phase or the display fix above.
  const { data: pr, error } = await db.from('procurement_requests').select(PR_LIST_COLUMNS + ',attachments').eq('id', id).single();
  if (error || !pr) {
    showLoader(false);
  ...
  document.getElementById('prModalBody').innerHTML =
    ...
    renderEngineerAttachments(currentPR) +
    rollupBannerHTML +
    groupingPanelHTML +
